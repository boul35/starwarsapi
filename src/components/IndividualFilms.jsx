...
const IndividualFilm = () => {
  ...
  if (data) {
    ...
    {starships.map(starship => <Card className="cards" style={{ width: '18rem' }}> <Card.Body> <Card.Title>{starship.name}</Card.Title><h2>{starship.model}</h2> <h2>Made By :  {starship.manufacturer}</h2> </Card.Body> </Card> )}
            
    </CardGroup>
    <h1>People</h1>
    <CardGroup className="cards" >
    
    {characters.map(character => <Card className="cards" style={{ width: '18rem' }}> <Card.Body> <Card.Title>{character.name}</Card.Title><h2>{character.birth_year}</h2></Card.Body> </Card> )}
            
    </CardGroup>    
    <h1>Vehicles</h1>  
    <CardGroup className="cards" >
    {vehicles.map(vehicle => <Card className="cards" style={{ width: '18rem' }}> <Card.Body> <Card.Title>{vehicle.name}</Card.Title><h2>{vehicle.model}</h2><h2>{vehicle.manufacturer}</h2></Card.Body> </Card> )}
